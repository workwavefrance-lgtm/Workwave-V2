/**
 * Cold NA - Mail 1 (presentation) - BATCH 4
 *
 * Equivalent du script batch3, pour la liste id=10 "Pros NA - Step 1 - Batch 4".
 * Tracking idempotent dedie : scripts/.brevo-cold-batch4-sent.json
 *
 * Usage:
 *   npx tsx scripts/brevo-cold-batch4.ts                # dry-run
 *   npx tsx scripts/brevo-cold-batch4.ts --test         # 1 mail a workwave.france@gmail.com
 *   npx tsx scripts/brevo-cold-batch4.ts --execute      # envoie aux pros eligibles
 *   npx tsx scripts/brevo-cold-batch4.ts --execute --limit 100
 */
...
const SENT_LOG_PATH = path.resolve(
  process.cwd(),
  "scripts/.brevo-cold-batch4-sent.json"
);
...
const LIST_ID = 10;
const LIST_NAME = "Pros NA - Step 1 - Batch 4";
...
async function main() {
  console.log("==========================================================");
  console.log("Cold NA - Mail 1 (presentation) - BATCH 4");
  console.log("==========================================================");
  ...
  for (const [i, t] of targets.entries()) {
    ...
    try {
      await brevo("POST", "/smtp/email", {
        ...
        htmlContent: html,
        headers: {
          "X-Mailin-Tag": "cold-batch4-resend",
  ...
  if (remaining > 0 && !TEST_MODE) {
    console.log(`  -> npx tsx scripts/brevo-cold-batch4.ts --execute  (relance pour finir)`);
  }
}
