/**
 * Libelles officiels des codes d'activite (NAF rev. 2, INSEE).
 *
 * POURQUOI CE FICHIER EXISTE. Mesure du 17/08/2026 : deux artisans du meme
 * metier dans la meme ville partageaient 80 % de texte identique sur leur
 * fiche, ce qui empechait Google d'indexer la seconde (218 870 pages en
 * "exploree, actuellement non indexee"). Le code d'activite est deja en base
 * sur 2 050 350 fiches et n'etait pas affiche : c'est la donnee gratuite qui
 * varie le plus d'une entreprise a l'autre.
 *
 * SOURCE : www.insee.fr/fr/statistiques/fichier/2120875/naf2008_liste_n5.xls
 * (nomenclature publique, 732 sous-classes). Table FIGEE : aucun appel
 * exterieur, aucun cout, et la nomenclature ne bouge pas.
 *
 * Les cles sont SANS point ("4399C"), comme la colonne pros.naf_code.
 * Genere par scripts/generer-libelles-naf.ts, ne pas editer a la main.
 */
...
/** Libelle officiel d'un code d'activite, ou null si le code est inconnu. */
export function libelleNaf(code: string | null | undefined): string | null {
  if (!code) return null;
  return LIBELLES_NAF[code.replace(/\./g, "").toUpperCase()] || null;
}
