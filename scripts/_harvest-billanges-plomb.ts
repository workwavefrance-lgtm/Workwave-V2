/**
 * Harvest ciblé : plombiers autour de Les Billanges (Haute-Vienne, dept 87) via
 * Apify Google Maps → table `prospects`. Déclenché par le lead de Michèle (08/06).
 * Mobiles 06/07 uniquement, filtre code postal "87", dédup interne. Idempotent.
 *   npx tsx scripts/_harvest-billanges-plomb.ts            # DRY-RUN
 *   npx tsx scripts/_harvest-billanges-plomb.ts --execute  # insère dans prospects
 */
...
const DRY = !EXECUTE;
const DEPT = "87";
const CAT_SLUG = "plombier";
const QUERIES = [
  "plombier Les Billanges",
  "plombier Ambazac",
  "plombier Bessines-sur-Gartempe",
  "plombier Saint-Léonard-de-Noblat",
  "plombier Nantiat",
  "plombier Razès",
];
...
async function main() {
  ...
  const harvested = items
    ...
    .filter((r) => r.postal.startsWith("87")) // vrais Haute-Vienne uniquement
    .filter((r) => { if (seen.has(r.phone!)) return false; seen.add(r.phone!); return true; });

  console.log(`→ ${harvested.length} plombiers avec MOBILE (06/07) en Haute-Vienne (87) :\n`);
  ...
  else console.log(`\n✓ ${count ?? rows.length} prospects insérés (plombier, dept 87).`);
}
