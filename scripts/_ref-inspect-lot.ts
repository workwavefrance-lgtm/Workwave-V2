...
(async () => {
  ...
  const urls: string[] = JSON.parse(fs.readFileSync(process.argv[2], "utf8"));
  ...
  for (const u of urls) {
    try {
      ...
      console.log(`${etat.padEnd(34)} | crawl=${r.lastCrawlTime ? r.lastCrawlTime.slice(0,10) : "jamais"} | ${u.replace("https://workwave.fr","")}`);
    } catch (e) { console.log(`ERREUR ${u} ${(e as Error).message.slice(0,80)}`); }
    await new Promise(r => setTimeout(r, 250));
  }
  console.log("\n=== recapitulatif ===");
  const tot = Object.values(compte).reduce((a, b) => a + b, 0);
  for (const [k, v] of Object.entries(compte).sort((a,b)=>b[1]-a[1])) console.log(`${v}/${tot} (${(100*v/tot).toFixed(0)}%) ${k}`);
})();
