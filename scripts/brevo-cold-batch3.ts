/**
 * Cold NA - Mail 1 (presentation) - BATCH 3
 *
 * Rejoue le Batch 3 (campagne Brevo #4, mis en pause par Willy le 28/04)
 * en pre-rendant le HTML cote script avec :
 *   - le NOM commercial du pro
 *   - l'URL DIRECTE vers sa fiche (https://workwave.fr/artisan/SLUG)
 *   - l'URL DIRECTE vers la reclamation (https://workwave.fr/pro/reclamer/SLUG)
 *
 * Format du mail = identique au Mail 1 original (campagne #2 envoyee le 27/04
 * qui a fait 20,22% de CTR vs 13,48% pour le mail generique). Aucun
 * placeholder Brevo {{...}}, donc impossible de rebuger.
 *
 * Liste Brevo : id=9 "Pros NA - Step 1 - Batch 3" (191 contacts)
 *
 * Usage:
 *   npx tsx scripts/brevo-cold-batch3.ts                # dry-run (defaut)
 *   npx tsx scripts/brevo-cold-batch3.ts --test         # 1 mail a workwave.france@gmail.com
 *   npx tsx scripts/brevo-cold-batch3.ts --execute      # envoie aux 191
 *   npx tsx scripts/brevo-cold-batch3.ts --execute --limit 50  # max 50 envois
 *
 * Idempotent via scripts/.brevo-cold-batch3-sent.json (peut s'arreter et reprendre).
 */
...
const SENT_LOG_PATH = path.resolve(
  process.cwd(),
  "scripts/.brevo-cold-batch3-sent.json"
);
...
const LIST_ID = 9; // Pros NA - Step 1 - Batch 3
const LIST_NAME = "Pros NA - Step 1 - Batch 3";
        ...
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0" style="max-width:600px;background:#ffffff;border-radius:8px;padding:32px 28px;">
          <tr>
            <td style="font-size:15px;color:#1a1a1a;line-height:1.7;">
              ...
              <p style="margin:0 0 16px;">
                Votre entreprise <strong>${safeName}</strong> est déjà référencée gratuitement sur notre site :
              </p>

              <p style="margin:0 0 24px;">
                <a href="${proUrl}" style="color:#FF5A36;text-decoration:underline;">${proUrl}</a>
              </p>

              <p style="margin:0 0 16px;">
                <strong>Petite alerte importante :</strong> votre fiche a été créée automatiquement à partir des données publiques du registre Sirene. Téléphone, adresse, horaires... certaines infos peuvent être obsolètes ou erronées. Vos clients potentiels qui tombent sur la fiche n'arrivent peut-être pas à vous joindre.
              </p>

              <p style="margin:0 0 16px;">
                Vérifiez et corrigez votre fiche en 3 minutes pour que les particuliers de votre zone vous trouvent (et vous contactent) vraiment :
              </p>
              ...
              <p style="margin:0 0 16px;">
                C'est gratuit, sans engagement, et boost votre référencement local sur Google.
              </p>
              ...
              <div style="border-top:1px solid #E5E7EB;padding-top:16px;margin-top:32px;font-size:11px;color:#9CA3AF;line-height:1.6;">
                ...
                <p style="margin:0;">
                  <a href="${unsubUrl}" style="color:#9CA3AF;text-decoration:underline;">Se désinscrire</a> · Workwave SAS, 3 rue des Rosiers 86110 Craon
                </p>
              </div>
            </td>
          </tr>
...
async function main() {
  console.log("==========================================================");
  console.log("Cold NA - Mail 1 (presentation) - BATCH 3");
  console.log("==========================================================");
  if (DRY_RUN) console.log("MODE : DRY-RUN (ajouter --execute pour envoyer)");
  if (TEST_MODE)
    console.log(`MODE : TEST (envoie 1 mail a ${ADMIN_TEST_EMAIL})`);
  if (!DRY_RUN && !TEST_MODE)
    console.log("MODE : EXECUTE (envoie pour de vrai)");
  console.log();

  // 1. Recuperer les contacts de la liste Batch 3
  ...
  // 2. Match avec Supabase pour avoir slug + name
  ...
  console.log(
    `  Pros DEJA CLAIM    : ${alreadyClaimed.length} (skipped pour pas spammer)`
  );

  // 3. Filtrer : seulement ceux avec slug ET pas encore claim
  const eligible = matched.filter(
    (m) => m.slug && !m.claimed_by_user_id
  );
  ...
  for (const m of eligible.slice(0, 3)) {
    ...
    console.log(
      `    -> https://workwave.fr/artisan/${m.slug}  (${m.name})`
    );
  }
  ...
  if (DRY_RUN) {
    ...
    console.log(
      "Ajouter --execute pour envoyer pour de vrai, ou --test pour 1 envoi a " +
        ADMIN_TEST_EMAIL
    );
    return;
  }

  // 5. Idempotence : skip emails deja envoyes precedemment
  console.log("\n[4/4] Envoi via Transactional API...");
  const alreadySent = loadSentEmails();
  if (alreadySent.size > 0) {
    console.log(
      `  ${alreadySent.size} emails deja envoyes precedemment, skipped.`
    );
  }
    ...
    pool = [
      {
        ...
        slug: eligible[0]?.slug || "atsaf",
        name: eligible[0]?.name || "ATSAF",
        claimed_by_user_id: null,
      },
    ];
  } else {
  ...
  console.log(
    `  ENVOI a ${targets.length} destinataires` +
      (pool.length > targets.length
        ? ` (${pool.length - targets.length} reportes a un prochain run)`
        : "") +
      "..."
  );
  ...
  for (const [i, t] of targets.entries()) {
    ...
    try {
      await brevo("POST", "/smtp/email", {
        ...
        htmlContent: html,
        headers: {
          "X-Mailin-Tag": "cold-batch3-resend",
  ...
  if (remaining > 0 && !TEST_MODE) {
    console.log(
      `  -> npx tsx scripts/brevo-cold-batch3.ts --execute  (relance pour finir)`
    );
  }
}
