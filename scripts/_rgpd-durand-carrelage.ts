/**
 * RGPD : suppression complete fiche michel-durand-00013 (Durand Carrelage)
 *
 * Demande formelle d'effacement de Freddy DURAND (contact@durandcarrelage.com)
 ...
 *
 * Run en 2 etapes :
 *   1. AUDIT  : npx tsx scripts/_rgpd-durand-carrelage.ts
 *   2. APPLY  : npx tsx scripts/_rgpd-durand-carrelage.ts --apply
 */
...
const SLUG = "michel-durand-00013";
const EMAIL = "contact@durandcarrelage.com";
...
async function main() {
  ...
  if (!APPLY) {
    ...
    console.log("  npx tsx scripts/_rgpd-durand-carrelage.ts --apply");
    console.log("\nActions :");
    ...
    console.log(`  2. UPSERT email_blacklist (email='${EMAIL}', reason='rgpd_deletion_request_2026-05-01')`);
  ...
  const { error: blErr } = await supabase
    ...
    .upsert(
      { email: EMAIL, reason: "rgpd_deletion_request_2026-05-01" },
      { onConflict: "email" }
    );
  if (blErr) {
    console.error("Erreur blacklist :", blErr);
  } else {
    console.log(`✓ ${EMAIL} ajoute a email_blacklist (reason=rgpd_deletion_request_2026-05-01)`);
  }
