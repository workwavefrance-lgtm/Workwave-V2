...
await p.screenshot({ path: `${OUT}/apres-1.png` });
...
await p.screenshot({ path: `${OUT}/apres-2.png` });

console.log("  clic « Maçon » :", await clic("Maçon"));
await pause(500);
console.log("  validation du/des metier(s) :", await p.evaluate(() => {
  const b = [...document.querySelectorAll("button")].find((x) => x.offsetParent !== null && x.textContent.includes("Continuer avec") || (x.offsetParent !== null && x.textContent.trim() === "Continuer →" && x.closest("div")?.previousElementSibling));
  const c = [...document.querySelectorAll("button")].find((x) => x.offsetParent !== null && /Continuer/.test(x.textContent));
  if (c) { c.click(); return true; } return false;
}));
await pause(800); console.log("ecran 3 :", await etape());
await p.screenshot({ path: `${OUT}/apres-3.png` });
...
await pause(500);
await p.screenshot({ path: `${OUT}/apres-4.png` });
...
await pause(1400);
await p.screenshot({ path: `${OUT}/apres-5.png` });
