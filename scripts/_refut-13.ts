...
(async () => {
  ...
  const seen=new Set<number>(); let last=0, batches=0;
  while(true){
    ...
    last=rows[rows.length-1].id; batches++;
  }
  ...
  console.log(`dept 13 KEYSET : ${cities.length} communes, ${totO} ouverts / ${totC} fermes, distinctes ${seen.size}, ${batches} lots`);
  console.log(`  couples ${all.size} | 0-ouvert+ferme ${zero} (${(100*zero/all.size).toFixed(1)}%)`);
  console.log(`  AUDIT annoncait : couples 2661 | 740 (27.8%)`);
