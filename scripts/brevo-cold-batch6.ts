/**
 * Cold NA - Mail 1 (presentation) - BATCH 6
 *
 * Source : Supabase directement (pas une liste Brevo).
 * Cible les ~381 pros eligibles qui ne sont PAS dans les listes Brevo
 * 9, 10, 11 (deja contactes par batches 3-5 hier soir).
 *
 * Filtres durs :
 *   - email NOT NULL
 *   - claimed_by_user_id IS NULL (pas deja inscrits)
 *   - do_not_contact = false (n'a pas opt-out)
 *   - email_bounced = false (pas de bounces precedents)
 *   - email NOT IN email_blacklist (RGPD requests, etc.)
 *   - email NOT IN listes Brevo 9, 10, 11 (deja mailed hier)
 *   - is_active = true ET deleted_at IS NULL
 *
 * Tracking idempotent : scripts/.brevo-cold-batch6-sent.json
 *
 * Usage :
 *   npx tsx scripts/brevo-cold-batch6.ts                # dry-run
 *   npx tsx scripts/brevo-cold-batch6.ts --test         # 1 mail a workwave.france@gmail.com
 *   npx tsx scripts/brevo-cold-batch6.ts --execute      # envoie aux eligibles
 *   npx tsx scripts/brevo-cold-batch6.ts --execute --limit 100
 */
...
const SENT_LOG_PATH = path.resolve(
  process.cwd(),
  "scripts/.brevo-cold-batch6-sent.json"
);
...
const SUBJECT_FALLBACK = "Votre fiche Workwave est en ligne";

// Listes Brevo deja contactees hier soir (batch3, 4, 5)
const ALREADY_CONTACTED_LISTS = [9, 10, 11];
...
async function getEmailsFromBrevoLists(listIds: number[]): Promise<Set<string>> {
  const all = new Set<string>();
  for (const listId of listIds) {
    let offset = 0;
    while (true) {
      const r = await brevo<{ contacts?: Array<{ email: string }> }>(
        "GET",
        `/contacts/lists/${listId}/contacts?limit=500&offset=${offset}`
      );
      const contacts = r.contacts ?? [];
      for (const c of contacts) if (c.email) all.add(c.email.toLowerCase());
...
async function getEligiblePros(): Promise<Pro[]> {
  // Pagination 1000 par 1000 (cap PostgREST par defaut)
  ...
  while (true) {
    const { data, error } = await supabase
      .from("pros")
      .select("id, email, slug, name, claimed_by_user_id, do_not_contact, email_bounced")
      .not("email", "is", null)
    ...
    if (error) {
      console.error("Erreur fetch pros :", error.message);
      break;
    }
    const rows = (data || []) as (Pro & { do_not_contact: boolean | null; email_bounced: boolean | null })[];
    if (rows.length === 0) break;
    for (const r of rows) {
      if (!r.email || !r.slug) continue;
      // double-check au cas ou les .or filters ne matchent pas exactement
      if (r.do_not_contact === true || r.email_bounced === true) continue;
      all.push({
        id: r.id,
        email: r.email,
        slug: r.slug,
...
async function getBlacklistEmails(): Promise<Set<string>> {
  const { data } = await supabase.from("email_blacklist").select("email");
  return new Set((data || []).map((r) => r.email.toLowerCase()));
}

async function main() {
  console.log("==========================================================");
  console.log("Cold NA - Mail 1 (presentation) - BATCH 6 (source = Supabase)");
  console.log("==========================================================");
  ...
  console.log("[1/5] Recup pros eligibles depuis Supabase...");
  const allEligible = await getEligiblePros();
  console.log(`  Pros eligibles total : ${allEligible.length}`);

  console.log("\n[2/5] Recup emails deja contactes (Brevo lists 9, 10, 11)...");
  const alreadyContacted = await getEmailsFromBrevoLists(ALREADY_CONTACTED_LISTS);
  console.log(`  Deja contactes hier  : ${alreadyContacted.size}`);

  console.log("\n[3/5] Recup blacklist (RGPD, opt-outs, bounces)...");
  const blacklist = await getBlacklistEmails();
  console.log(`  Blacklist            : ${blacklist.size}`);

  // Filtre final
  const eligible = allEligible.filter(
    (p) =>
      !alreadyContacted.has(p.email.toLowerCase()) &&
      !blacklist.has(p.email.toLowerCase())
  );
  console.log(`\n  Cible BATCH 6        : ${eligible.length} pros`);

  console.log("\n[4/5] Apercu 3 premiers :");
  for (const m of eligible.slice(0, 3)) {
    console.log(`  ${m.email.padEnd(40)} -> ${m.slug} (${m.name})`);
  }
  ...
  console.log("\n[5/5] Envoi via Transactional API...");
  ...
  if (remaining > 0 && !TEST_MODE) {
    console.log(`  -> npx tsx scripts/brevo-cold-batch6.ts --execute  (relance pour finir)`);
  }
}
