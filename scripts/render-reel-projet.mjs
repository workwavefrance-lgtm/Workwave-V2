 ...
 *
 * Usage :
 *   node scripts/render-reel-projet.mjs marketing/projets/<projet>.json
 *
 * Le JSON projet :
 *   { "slug": "macon-marsolan", "theme": "light"|"dark", "metier": "Maçon",
 *     "ville": "Marsolan", "dept": "Gers (32)",
...
if (!dataPath) { console.error("Usage: node scripts/render-reel-projet.mjs <projet.json>"); process.exit(1); }
const project = JSON.parse(fs.readFileSync(dataPath, "utf8"));
if (!project.slug || !project.metier) { console.error("JSON invalide: slug + metier requis"); process.exit(1); }

// Injecte les données pour le template
fs.writeFileSync(
  path.resolve("marketing/reel-projet-data.js"),
...
const OUT = process.env.REEL_FRAMES_DIR || path.join(os.tmpdir(), "workwave-frames-projet");
const FPS = 30;
...
await page.goto("http://localhost:8877/reel-projet-render.html", { waitUntil: "networkidle0" });

const total = await page.evaluate(() => window.TOTAL);
const nFrames = Math.ceil(total / (1000 / FPS));
console.log(`[${project.slug}] theme=${project.theme} · ${total}ms → ${nFrames} frames`);
