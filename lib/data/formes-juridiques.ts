/**
 * Libelles officiels des categories juridiques (niveau III, INSEE).
 *
 * POURQUOI. La forme juridique (entreprise individuelle, SARL, SAS...) change
 * d'une entreprise a l'autre y compris entre deux voisins du meme metier
 * dans la meme ville. C'est donc une des rares donnees gratuites capables de
 * distinguer deux fiches, ce qui est l'enjeu mesure le 17/08/2026 : 80 % de
 * texte identique entre voisins et 218 870 pages "exploree, actuellement non
 * indexee".
 *
 * SOURCE : www.insee.fr/fr/statistiques/fichier/2028129/cj_septembre_2022.xls
 * (nomenclature publique). Table FIGEE : aucun appel exterieur, aucun cout.
 *
 * Genere par scripts/generer-libelles-juridiques.ts, ne pas editer a la main.
 */
...
/** Libelle officiel d'une categorie juridique, ou null si le code est inconnu. */
export function libelleFormeJuridique(code: string | null | undefined): string | null {
  if (!code) return null;
