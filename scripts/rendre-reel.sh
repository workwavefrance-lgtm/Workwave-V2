...
case "$TYPE" in
  projet)    SCRIPT=scripts/render-reel-projet.mjs;     DIR=marketing/frames-projet;      PREFIXE=Workwave-projet ;;
  bienvenue) SCRIPT=scripts/render-reel-nouveau-pro.mjs; DIR=marketing/frames-nouveau-pro; PREFIXE=Workwave-nouveau-pro ;;
...
REEL=$(ls "$DIR"/f*.png 2>/dev/null | wc -l | tr -d ' ')
if [ "$REEL" != "$ATTENDU" ]; then
  echo "ECHEC $SLUG : $REEL images sur le disque pour $ATTENDU attendues"; exit 1
fi
# La suite doit etre CONTIGUE : ffmpeg s'arrete au premier trou.
DERNIERE=$(printf "f%05d.png" $((ATTENDU - 1)))
[ -f "$DIR/$DERNIERE" ] || { echo "ECHEC $SLUG : image finale $DERNIERE absente"; exit 1; }
